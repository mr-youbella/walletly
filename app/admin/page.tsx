...
export default async function AdminPage() {
	...
	return (
		<div className="relative min-h-screen w-full overflow-hidden bg-linear-to-br from-[#0a0b10] via-[#0f0f1a] to-[#1a0f0f] px-6 py-10 text-white">
			<div
			...
			<main className="relative mx-auto max-w-4xl">
				<div className="flex items-center gap-3 p-4 rounded-2xl bg-linear-to-r from-red-500/5 to-orange-500/5 border border-white/5">
					<div
						className="flex h-9 w-9 items-center justify-center rounded-xl shadow-lg shadow-red-500/20"
						style={{ background: "linear-gradient(135deg, #DC2626, #991B1B)" }}
					>
						<ShieldCheck size={18} className="text-white" />
					</div>
					<div>
						<h1 className="text-xl font-bold bg-linear-to-r from-[#DC2626] to-[#F97316] bg-clip-text text-transparent">Admin</h1>
						<p className="text-xs text-white/40">Signed in as @{currentLogin}</p>
					</div>
				</div>
				...
				<div className="mt-4 overflow-hidden rounded-2xl border border-white/6 bg-linear-to-br from-white/2 to-red-500/2 backdrop-blur-sm">
					<table className="w-full text-left text-sm">
						...
						<tbody>
							{logins.map((row) => (
								<tr key={row.id} className="border-b border-white/4 last:border-0 transition-colors hover:bg-red-500/5">
									<td className="flex items-center gap-2 px-5 py-3 font-medium">
										<Flame size={14} className="text-[#DC2626]" />
										<a href={`https://profile.intra.42.fr/users/${row.login}`} className="text-white/90">@{row.login}</a>
									</td>
									...
									<td className="px-5 py-3 text-white/50">{formatDate(row.first_login_at)}</td>
									<td className="px-5 py-3 text-white/50">{formatDate(row.last_login_at)}</td>
								</tr>
							))}
